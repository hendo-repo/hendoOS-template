...
Commands: orient, gate, reference, receipts, serve
...
Options: --config FILE (owner JSON policy); --input-timeout-ms 1..30000 (default 5000).
...
export async function main(args: string[]): Promise<number> {
  ...
  try {
    ...
    if (!command || !['orient', 'gate', 'reference', 'receipts', 'serve'].includes(command)) throw new Error('invalid command');
    const flags = new Map<string, string>();
    const accepted = new Set(['--state', '--content', '--config', '--request', '--owner', '--session', '--limit', '--input-timeout-ms']);
    ...
    if (!flags.get('--state') || !flags.get('--content')) throw new Error('explicit --state and --content required');
    const specific = command === 'receipts' ? ['--owner', '--session', '--limit'] : command === 'serve' ? [] : ['--request', '--input-timeout-ms'];
    for (const key of flags.keys()) if (!['--state', '--content', '--config', ...specific].includes(key)) throw new Error('option not valid for command');
