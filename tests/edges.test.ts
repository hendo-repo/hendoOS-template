...
function op(extra: Record<string, unknown> = {}) {
  return { version: 1, schemaVersion: 1, composeVersion: 1, contentGeneration: 2, requestId: 'request', sessionId: 'session', ownerId: 'owner', nonce: 'nonce',
    command: 'gate', scenarioId: 'pre-edit-kernel-plus-declared-reference', subjectDigest: digest,
    configRevision: DEFAULT_CONFIG.revision, checkerRevision: 'aos-policy/1', observations: [{ key: 'verification', status: 'fresh', value: true,
      provenance: { kind: 'synthetic', source: 'test', subjectDigest: digest, configRevision: DEFAULT_CONFIG.revision, checkerRevision: 'aos-policy/1' } }], ...extra };
}
...
async function run(dir: string, command: string, input: string, extra: string[] = []) {
  const child = Bun.spawn([process.execPath, 'src/edges/cli.ts', command, '--state', join(dir, 'state.sqlite'), '--content', resolve('content'), ...extra],
...
test('help is runnable without homes/config/state', async () => {
  ...
  for (const command of ['orient', 'gate', 'reference', 'receipts', 'serve']) expect(text).toContain(command);
});
...
test('stdio handshake, discovery, resource reads and notifications are standard JSON-RPC', async () => {
  ...
  expect(messages.find(m => m.id === 2).result.tools.map((t: { name: string }) => t.name)).toEqual(['orient', 'gate', 'reference']);
...
test('input deadline closes real stdin wait without spawning children', async () => {
  const dir = setup();
  const child = Bun.spawn([process.execPath, 'src/edges/cli.ts', 'gate', '--state', join(dir, 'state.sqlite'), '--content', resolve('content'), '--input-timeout-ms', '10'],
...
test('AOS initialize generation mismatch refuses before tools are available', async () => {
  const result = await run(setup(), 'serve', wire({ ...init, params: { ...init.params as object,
    capabilities: { experimental: { aos: { version: 1, schemaVersion: 1, composeVersion: 1, contentGeneration: 0 } } } } }));
